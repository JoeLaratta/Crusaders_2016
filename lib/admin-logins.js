...
// POST    bulk     { action: "bulk" }                 -> one login per player
//                                                        that has none, temp
//                                                        passwords returned ONCE
...
// Player name -> username. Strips apostrophes before collapsing punctuation so
// O'Brien becomes obrien, and drops bracketed alternates so
// "Denham Weeks (Longo)" becomes denham.weeks rather than denham.weeks.longo.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- bulk generation -------------------------------------------------
    // One login per player that does not already have one. Safe to run more
    // than once: players with an existing login are skipped, so nobody's
    // password is reset by accident. Temp passwords come back once only.
    if (method === 'POST' && body.action === 'bulk') {
      const orphans = await sql`
        select p.id, p.name from players p
        where not exists (
          select 1 from login_players lp where lp.player_id = p.id
        )
        order by p.name
      `;
      if (orphans.length === 0) {
        return res.status(200).json({ created: [], skipped: 0, message: 'Every player already has a login.' });
      }

      // Pull existing usernames once so suffixing does not need a query per player.
      const takenRows = await sql`select username from logins`;
      const taken = {};
      takenRows.forEach(function (r) { taken[r.username] = true; });

      const created = [];
      for (const p of orphans) {
        let base = usernameFromName(p.name);
        if (!base) base = 'player';
        let username = base;
        let n = 2;
        ...
        await setPlayers(newId, [Number(p.id)]);
